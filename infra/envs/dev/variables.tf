...
variable "enable_nat_gateway" {
  description = "Whether to create NAT Gateways (false in dev to save cost)"
  type        = bool
  default     = false
}
...
variable "backend_image" {
  description = "Docker image for the backend container (e.g. nginx:alpine)"
  type        = string
}
...
variable "backup_retention_period" {
  description = "Days to retain automated RDS backups (dev: 1)"
  type        = number
}
